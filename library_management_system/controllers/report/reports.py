...
def reports():
    top_books_by_rating = Books.query.order_by(Books.average_rating.desc()).limit(5).all()
    newest_books = Books.query.order_by(Books.publication_date.desc()).limit(5).all()
    top_members_by_amount_spent = Members.query.order_by(Members.amount_spent.desc()).limit(5).all()
    top_members_by_outstanding_amount = Members.query.order_by(Members.amount_spent.desc()).limit(5).all()
    popular_books = Transactions.query.with_entities(
        Books, Transactions.book_id, func.count(Transactions.id)
    ).join(
        Books,
        Transactions.book_id == Books.id
    ).group_by(Transactions.book_id).order_by(-1*func.count(Transactions.id)).all()

    books_with_min_copies_remaining = Books.query.order_by(
        (Books.quantity - Books.issued).asc()
    ).limit(5).all()
