...
class DeleteMemberTestCase(unittest.TestCase):
    ...
    def test_delete_member(self):
        with self.app.application.app_context():
            ...
            # Edit member
            edit_data = {"name": "Test1", "email": "test@flask.com", "outstanding_debt": 0, "amount_spent": 0}
            edit_member_response = self.app.post(f"/member/edit/{member.id}", data=edit_data, follow_redirects=True)
            self.assertEqual(edit_member_response.status_code, 200)

            # Assert that the member was edited in the database
            member: Members = db.session.query(Members).filter(Members.email == data['email']).one()
            self.assertEqual(member.name, "Test1")
